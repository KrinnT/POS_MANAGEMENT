...
export default function SmartOrderingPage({ params }: { params: { qrToken: string } }) {
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        const table = tables.find((t) => t.qrToken === params.qrToken);
        if (!table) {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-slate-600">Cart: <span className="font-semibold text-slate-900">{cartTotal.toFixed(0)}</span></div>
          </div>
